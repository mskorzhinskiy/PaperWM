...
function onWindowGapChanged() {
  setVerticalMargin();
  if (timerId) imports.mainloop.source_remove(timerId);
...
function findWorkspaceSettingsByName(regex) {
  ...
  return Extension.imports.utils
    .zip(
      list,
      settingss,
      settingss.map((s) => s.get_string("name"))
    )
...
function keycomboToKeystr(combo) {
  ...
  let key = mutterKey;
  if (mutterKey === META_KEY_ABOVE_TAB) key = 97; // a
...
function findConflicts(schemas) {
  ...
  let conflicts = [];
  const paperMap = generateKeycomboMap(
