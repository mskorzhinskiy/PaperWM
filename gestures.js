    ...
    switch (phase) {
      case Clutter.TouchpadGesturePhase.UPDATE:
        ...
        if (direction === DIRECTIONS.Vertical) {
          updateVertical(
            -dy * natural * prefs.swipe_sensitivity[1],
            event.get_time()
          );
...
function horizontalScroll(actor, event) {
  ...
  switch (phase) {
    case Clutter.TouchpadGesturePhase.UPDATE:
      ...
      return update(
        this,
        -dx * natural * prefs.swipe_sensitivity[0],
        event.get_time()
      );
    case Clutter.TouchpadGesturePhase.CANCEL:
    case Clutter.TouchpadGesturePhase.END:
...
function done(space) {
  ...
  if (newD < Math.abs(d)) t = t * Math.abs(newD / d);
  ...
  Tweener.addTween(space.cloneContainer, {
    ...
    onComplete: () => {
      if (!Tiling.inPreview) Navigator.getNavigator().finish();
    },
  });
}
...
function findTargetWindow(space, direction) {
  ...
  if (direction) {
    // ->
    r1 =
      Math.abs(closest.targetX + closest.width + space.targetX) / closest.width;
    r2 = Math.abs(next.targetX + space.targetX - space.width) / next.width;
  } else {
    r1 =
      Math.abs(closest.targetX + space.targetX - space.width) / closest.width;
...
function updateVertical(dy, t) {
  ...
  if (
    ...
    Tiling.spaces.selectStackSpace(
...
function endVertical() {
  ...
  let glide = () => {
    if (vState < Clutter.TouchpadGesturePhase.END) return false;
