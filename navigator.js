...
var Navigator = class Navigator {
  ...
  destroy(space, focus) {
    this.minimaps.forEach((m) => {
      if (typeof m === "number") Mainloop.source_remove(m);
      else m.destroy();
    });
    ...
    for (let monitor of Main.layoutManager.monitors) {
      ...
      if (monitor === this.monitor) continue;
    ...
    if (this.space === from) {
      ...
      if (Tiling.inGrab && Tiling.inGrab.window) {
        this.space.workspace.activate_with_focus(
    ...
    if (selected && !Tiling.inGrab) {
      ...
      selected.foreach_transient(
        (mw) => (hasFocus = mw.has_focus() || hasFocus)
      );
      if (hasFocus) {
