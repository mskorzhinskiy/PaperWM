...
function gotoByIndex() {
  ...
  for (let k = 1; k <= 9; k++) {
    Keybindings.bindkey(`<Super>${k}`, `goto-coloumn-${k}`, goto(k - 1), {
...
function windowMarks() {
    ...
    return (metaWindow, space, options) => {
      ...
      if (mark.has_focus()) {
        ...
        if (!options.navigator) {
          let mru = global.display.get_tab_list(Meta.TabList.NORMAL_ALL, null);
          let nextWindow = mru[1];
          if (!nextWindow) return;
          Main.activateWindow(nextWindow);
          if (
            Scratch.isScratchWindow(mark) &&
  ...
  for (let k = 0; k <= 9; k++) {
    Keybindings.bindkey(`<Super>${k}`, `goto-mark-${k}`, gotoMark(k), {
      activeInNavigator: true,
    });
    Keybindings.bindkey(`<Super><Shift>${k}`, `set-mark-${k}`, setMark(k), {
      activeInNavigator: true,
    });
  }
}
...
function cycleWorkspaceSettings(binding = "<Super>q") {
  ...
  Keybindings.bindkey(
    binding,
    "next-space-setting",
    (mw) => Tiling.cycleWorkspaceSettings(-1),
    { activeInNavigator: true }
  );
  Keybindings.bindkey(
    "<Shift>" + binding,
...
function showNavigator(binding = "<Super>j") {
  Keybindings.bindkey(binding, "show-minimap", () => null, {
    opensMinimap: true,
  });
}
...
function listFreeBindings(modifierString) {
  ...
  return []
    .concat(chars, symbols)
    .filter((key) => Keybindings.getBoundActionId(modifierString + key) === 0)
...
function adjustWidth(
  incBinding = "<Super>plus",
  decBinding = "<Super>minus",
  increment = 50
) {
...
function tileInto(
  leftBinding = "<Super><Shift>less",
  rightBinding = "<Super><Shift>less"
) {
...
function stackUnstack(basebinding = "<Super><Alt><Ctrl>") {
  ...
  const stackUnstackDirection =
    (dir = -1) =>
    (metaWindow) => {
      let space = Tiling.spaces.spaceOfWindow(metaWindow);
      let column_idx = space.indexOf(metaWindow);
      if (column_idx < 0) return;
      ...
      if (column.length >= 2) {
        ...
        if (column_idx == 0 && dir == -1) return;
        if (column_idx + 1 >= space.length && dir == 1) return;
      ...
      space.layout(true, {
        customAllocators: {
          [space.indexOf(metaWindow)]: Tiling.allocateEqualHeight,
        },
      });
      space.emit("full-layout");
    };
...
function reorderWorkspace(
  bindingUp = "<Alt><Super>Page_Up",
  bindingDown = "<Alt><Super>Page_Down"
) {
  ...
  function moveWorkspace(dir, metaWindow, space) {
    ...
    let nextI = Math.min(
      Tiling.spaces.size - 1,
      Math.max(0, space.workspace.index() + dir)
    );
