...
var Space = class Space extends Array {
  constructor(workspace, container, doInit) {
    ...
    if (oldSpace && oldSpace.monitor) {
      ...
      if (oldMonitor) monitor = oldMonitor;
    }
  ...
  init() {
    ...
    this.signals.connect(
      settings,
      "changed::use-default-background",
      this.updateBackground.bind(this)
    );
    this.signals.connect(
      backgroundSettings,
  ...
  hide() {
    ...
    this._visible = false;
    this.clip.hide();
    for (let col of this) for (let w of col) w.clone.cloneActor.source = null;
  }
  ...
  workArea() {
    ...
    let topBarAdjustment =
      this.showTopBar &&
      (prefs.topbar_follow_focus ||
  ...
  layoutGrabColumn(
    ...
    function mosh(windows, height, y0) {
      ...
      let [w, relayout, y] = space.layoutColumnSimple(
        windows,
        x,
        y0,
        targetWidth,
  ...
  layoutColumnSimple(windows, x, y0, targetWidth, targetHeights, time) {
    ...
    for (let i = 0; i < windows.length; i++) {
      ...
      let resizable =
        !mw.fullscreen && mw.get_maximized() !== Meta.MaximizeFlags.BOTH;

      if (resizable) {
        const hasNewTarget =
          mw._targetWidth !== targetWidth || mw._targetHeight !== targetHeight;
        const targetReached =
  ...
  layout(animate = true, options = {}) {
    ...
    for (let i = 0; i < this.length; i++) {
      ...
      if (inGrab && column.includes(inGrab.window) && !allocator) {
        [resultingWidth, relayout] = this.layoutGrabColumn(
          column,
          x,
          y0,
          targetWidth,
        ...
        [resultingWidth, relayout] = this.layoutColumnSimple(
          column,
          x,
          y0,
          targetWidth,
    ...
    let min = workArea.x;
    let auto =
      (this.targetX + oldWidth >= min + workArea.width && this.targetX <= 0) ||
  ...
  isVisible(metaWindow, margin = 0) {
    ...
    if (x - margin + clone.width < min || x + margin > min + workArea.width) {
      return false;
    } else {
      return true;
    }
  ...
  isPlaceable(metaWindow) {
    ...
    if (
      x + clone.width < min + stack_margin ||
      ...
      if (
        (metaWindow.get_maximized() === Meta.MaximizeFlags.BOTH && x !== min) ||
  ...
  getWindow(index, row) {
    if (row < 0 || index < 0 || index >= this.length) return false;
  ...
  addWindow(metaWindow, index, row) {
    ...
    this.signals.connect(metaWindow, "position-changed", (w) => {
      ...
      x = Math.min(
        this.width - stack_margin,
        Math.max(stack_margin - f.width, x)
      );
      x += this.monitor.x;
      if (f.x !== x || f.y !== y) {
  ...
  switchLinear(dir) {
    ...
    if (utils.in_bounds(column, row + dir) == false) {
      ...
      if (dir === 1) {
        ...
        if (index >= 0) row = this[index].length - 1;
      }
    } else {
      row += dir;
    }
  ...
  switchLeft() {
    this.switch(Meta.MotionDirection.LEFT);
  }
  switchRight() {
    this.switch(Meta.MotionDirection.RIGHT);
  }
  switchUp() {
    this.switch(Meta.MotionDirection.UP);
  }
  switchDown() {
    this.switch(Meta.MotionDirection.DOWN);
  }
  switch(direction) {
    let space = this;
    ...
    if (index < 0 || index >= space.length) return;
  ...
  positionOf(metaWindow) {
    ...
    let index, row;
    for (let i = 0; i < this.length; i++) {
      if (this[i].includes(metaWindow)) return [i, this[i].indexOf(metaWindow)];
    }
    return false;
  }
  ...
  moveDone() {
    ...
    this.getWindows().forEach((w) => {
      ...
      if (Tweener.isTweening(w.clone)) return;

      let unMovable =
        w.fullscreen || w.get_maximized() === Meta.MaximizeFlags.BOTH;
      if (unMovable) return;
...
`);
  ...
  updateBackground() {
    let path =
      this.settings.get_string("background") || prefs.default_background;
  ...
  createBackground() {
    ...
    this.signals.connect(
      ...
      (actor, event) => {
        ...
        if (windowAtPoint) {
          ...
          inGrab = new Extension.imports.grab.MoveGrab(
            windowAtPoint,
            Meta.GrabOp.MOVING,
            this
          );
    ...
    this.signals.connect(this.background, "scroll-event", (actor, event) => {
  ...
  setMonitor(monitor, animate) {
    ...
    Tweener.addTween(this.actor, { x: 0, y: 0, scale_x: 1, scale_y: 1, time });
    Tweener.addTween(clip, { scale_x: 1, scale_y: 1, time });
    ...
    this.border.set_size(
      monitor.width + scale * borderWidth * 2,
...
var Spaces = class Spaces extends Map {
  ...
  constructor() {
    ...
    if (global.screen) {
      ...
      // 3.30 now uses per desktop settings, instead of ad-hoc overrides
  ...
  monitorsChanged() {
    ...
    this.onlyOnPrimary = this.overrideSettings.get_boolean(
      "workspaces-only-on-primary"
    );

    if (this.monitors) oldMonitors = this.monitors;

    this.monitors = new Map();
    this.get(workspaceManager.get_active_workspace())
      .getWindows()
      .forEach((w) => {
        animateWindow(w);
      });
    ...
    let finish = () => {
      ...
      imports.mainloop.timeout_add(20, () => {
  ...
  switchMonitor(direction, move) {
    ...
    print(
      "switch",
      utils.ppEnumValue(direction, Meta.DisplayDirection),
      i,
      monitor.index
    );
  ...
  selectSequenceSpace(direction, move) {
    ...
    if (move && this.selectedSpace.selectedWindow) {
      takeWindow(this.selectedSpace.selectedWindow, this.selectedSpace, {
        navigator: Navigator.getNavigator(),
      });
    }

    if (direction === Meta.MotionDirection.DOWN) to = from + 1;
    else to = from - 1;

    if (to < 0 || to >= monitorSpaces.length) return;

    if (to === from && Tweener.isTweening(newSpace.actor)) return;
    ...
    monitorSpaces.forEach((space, i) => {
      ...
      if (to === 0) {
        ...
        space_y =
          center * 2 - padding + (space.height + padding) * (i - to) * scale;
      } else {
  ...
  selectStackSpace(direction, move) {
    ...
    mru.forEach((space, i) => {
      ...
      let h,
        onComplete = () => {};
      if (to === i) h = StackPositions.selected;
      else if (to + 1 === i) h = StackPositions.up;
      else if (to - 1 === i) h = StackPositions.down;
      else if (i > to) h = StackPositions.top;
      else if (i < to) h = StackPositions.bottom;
...
function insertWindow(metaWindow, { existing }) {
  ...
  let connectSizeChanged = (tiled) => {
    if (tiled) animateWindow(metaWindow);
    actor.opacity = 255;
    metaWindow.unmapped &&
  ...
  index++;
  if (!space.addWindow(metaWindow, index)) return;
...
function ensuredX(meta_window, space) {
  ...
  if (
    neighbour ||
    space.isVisible(meta_window) ||
    meta_window.lastFrame === undefined
  )
    x = Math.round(clone.targetX) + space.targetX;
  else x = meta_window.lastFrame.x - monitor.x;
  let gap = prefs.window_gap;
  let workArea = space.workArea();
  let min = workArea.x;
  ...
  if (meta_window.fullscreen) {
    ...
    frame.width >
    workArea.width * 0.9 - 2 * (prefs.horizontal_margin + prefs.window_gap)
  ) {
...
function ensureViewport(meta_window, space, force) {
  ...
  let index = space.indexOf(meta_window);
  if (index === -1 || space.length === 0) return undefined;

  debug("Moving", meta_window.title);

  if (space.selectedWindow.fullscreen && !meta_window.fullscreen) {
...
function updateSelection(space, metaWindow) {
  ...
  if (space.selection.get_parent() === clone) return;
...
function grabBegin(metaWindow, type) {
  switch (type) {
    ...
    case Meta.GrabOp.KEYBOARD_MOVING:
      ...
      if (inGrab.initialSpace.isFloating(metaWindow)) return;
...
function focus_handler(metaWindow, user_data) {
  debug(
    "focus:",
    metaWindow.title,
    utils.framestr(metaWindow.get_frame_rect())
  );
...
function showHandler(actor) {
  ...
  if (metaWindow.unmapped) {
    if (utils.version[1] < 34) animateWindow(metaWindow);
    else actor.opacity = 0;
    return;
  }
...
function resizeHInc(metaWindow) {
  ...
  let maxHeight =
    workArea.height - prefs.horizontal_margin * 2 - prefs.window_gap;
...
function resizeWInc(metaWindow) {
  ...
  let maxWidth =
    workArea.width - prefs.horizontal_margin * 2 - prefs.window_gap;
...
function cycleWindowWidth(metaWindow) {
  ...
  let targetWidth = Math.min(
    utils.findNext(frame.width, steps, sizeSlack),
    workArea.width
  );
  let targetX = frame.x;
...
function fitProportionally(values, targetSum) {
  ...
  let fitted = utils
    .zip(values, weights)
    .map(([h, w]) => Math.round(targetSum * w));
...
function slurp(metaWindow) {
...

  {
    ...
    column.splice(row, 1);
    if (column.length === 0) space.splice(from, 1);
  }
...
function takeWindow(metaWindow, space, { navigator }) {
  ...
  if (!space.removeWindow(metaWindow)) return;
  ...
  let point = space.cloneContainer.apply_relative_transform_to_point(
    parent,
    new Clutter.Vertex({ x: metaWindow.clone.x, y: metaWindow.clone.y })
  );
  ...
  animateWindow(metaWindow);
  Tweener.addTween(metaWindow.clone, { x, y, time: prefs.animation_time });
}
...
function rotated(list, dir = 1) {
  return [].concat(list.slice(dir), list.slice(0, dir));
}
...
function cycleWorkspaceSettings(dir = 1) {
  ...
  // in the last step
  let settings = uuids.map((uuid) => [
    uuid,
    Settings.getWorkspaceSettingsByUUID(uuid),
  ]);
