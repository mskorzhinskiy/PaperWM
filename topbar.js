...
`);
  ...
  this.prevIcon = createButton(
    "go-previous-symbolic",
    "previous workspace setting"
  );
...
class ColorEntry {
  constructor(startColor) {
    ...
    this.entry.actor.clutter_text.connect("text-changed", () => {
  ...
  class WorkspaceMenu extends PanelMenu.Button {
    _init() {
      ...
      this.menu.addMenuItem(
        new PopupMenu.PopupSeparatorMenuItem(_("Workspace Settings"))
      );
      ...
      let changed = () => {
        ...
        let space = Tiling.spaces.spaceOf(
      ...
      this._prefItem = new PopupMenu.PopupImageMenuItem(
    ...
    _onEvent(actor, event) {
      ...
      if (
        ["NORMAL", "SCROLL"].includes(this.state) &&
        ...
        if (!this._navigator) {
          ...
          this._enterbox.set_position(
            panelBox.x,
            panelBox.y + panelBox.height + 20
          );
          this._enterbox.set_size(global.screen_width, global.screen_height);
          Main.layoutManager.trackChrome(this._enterbox);

          this._navigator.connect(
            "destroy",
            this._finishWorkspaceSelect.bind(this)
          );
      ...
      if (
        ...
        const downEdge = 0.6 * active.height;
        if (
          dy > 0 &&
        ...
        if (this.selected === active) {
          ...
          let s =
            1 -
            (1 - scale) *
              (this.selected.actor.y / (0.1 * this.selected.height));
          s = Math.max(s, scale);
          Tweener.removeTweens(this.selected.actor);
        ...
        if (v === 0 && !firstEvent) {
          ...
          let test;
          if (this.velocity > 0) test = () => this.velocity > 0;
          else test = () => this.velocity < 0;
          ...
          while (test()) {
            ...
            if (this.velocity > 0) this.velocity -= friction;
            else this.velocity += friction;
          }
    ...
    _onOpenStateChanged(menu, open) {
      ...
      GLib.idle_add(
        ...
        this.entry.activate.bind(this.entry)
      );
    ...
    setName(name) {
      if (prefs.use_workspace_name) this._label.text = name;
      else this._label.text = orginalActivitiesText;
    }
  }
);
...
function enable() {
  ...
  function fixLabel(label) {
    ...
    for (let [workspace, space] of Tiling.spaces) {
      space.label.set_position(
        panel.x + Math.round(r.x),
        panel.y + Math.round(r.y)
      );
  ...
  signals.connect(Main.overview, "hidden", () => {
    if (Tiling.spaces.selectedSpace.showTopBar) return;
    fixTopBar();
  });

  signals.connect(
    Settings.settings,
    "changed::topbar-follow-focus",
    (settings, key) => {
...
function updateWorkspaceIndicator(index) {
  let spaces = Tiling.spaces;
  let space =
    spaces && spaces.spaceOf(workspaceManager.get_workspace_by_index(index));
  let onMonitor = space && space.monitor === panelMonitor;
  let nav = Navigator.navigator;
  if (
    onMonitor ||
    (Tiling.inPreview && nav && nav.from.monitor === panelMonitor)
  )
